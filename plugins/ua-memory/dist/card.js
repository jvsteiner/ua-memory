...
function build(primary, others, input, fileCap) {
    ...
    out.push("- Import and call edges come from tree-sitter parsing. Treat them as fact.");
    out.push("- Summaries, tags and layer names were written by an LLM. Treat them as hints and verify before relying on one.");
    out.push("- For more detail, run `ua-memory file <path>`, `ua-memory impact <path>` or `ua-memory layer <name>`. Never read the graph JSON directly — it is about 100,000 tokens.");
    return out.join("\n");
}
