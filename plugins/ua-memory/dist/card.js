...
function build(primary, others, input, fileCap) {
    ...
    if (project.languages?.length)
        out.push(`Languages: ${project.languages.join(", ")}`);
    if (project.frameworks?.length)
        out.push(`Frameworks: ${project.frameworks.join(", ")}`);
    out.push("");
    ...
    if (Number.isFinite(fileCap)) {
        out.push("File lists below are shortened to fit the 10,000 character limit on " +
    ...
    if (others.length > 0) {
        ...
        for (const other of others) {
            ...
            out.push(`- ${rel} — ${other.graph.layers.length} layers, ${other.fileCount} files. ` +
    ...
    out.push("- The import and call edges in this map were extracted by tree-sitter from the source.");
    out.push("- The summaries, tags and layer names in this map were written by an LLM and can be out of date.");
    out.push("- This map records no dynamic `import()` edges and no re-exports through a barrel file, so some real dependents are missing from it.");
    out.push("- The `ua-memory` command reports more: `ua-memory file <path>`, `ua-memory impact <path>`, `ua-memory layer <name>`. The graph JSON behind this card is about 100,000 tokens.");
    return out.join("\n");
}
function filesIn(map, layer) {
...
function freshnessLine(map, input) {
    const built = map.meta?.gitCommitHash;
    if (!built)
        return "Map built at an unknown commit — freshness cannot be checked.";
    const short = built.slice(0, 7);
    if (!input.head)
        return `Map built at ${short}. Current commit unknown, so it may be stale.`;
    if (input.head === built)
        return `Map built at ${short}, which is the current commit. It is up to date.`;
    ...
    return (`Map built at ${short}; HEAD is now ${head}. ` +
