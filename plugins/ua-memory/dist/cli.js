...
function parse(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--json")
            ...
            depth = Math.min(3, Math.max(1, Number(argv[++i]) || 1));
        else if (arg === "--limit")
...
function staleness(primary) {
    ...
    const changedFiles = repoRoot && head && built && head !== built
...
export function run(argv, cwd) {
    ...
    const emit = (value, text) => args.json ? { out: JSON.stringify(value, null, 2), code: 0 } : { out: text, code: 0 };
    switch (args.command) {
        case "card": {
        ...
        case "where": {
            ...
            if (!text)
                return { out: "ua-memory where <text>", code: 1 };
            const hits = where(primary, text, args.limit);
            if (hits.length === 0)
                return { out: `Nothing matches "${text}".`, code: 0 };
            return emit(hits, hits
        ...
        case "file": {
            ...
            if (!info)
                return { out: notFound(primary, args.rest[0] ?? ""), code: 1 };
            const lines = [
                info.path,
        ...
        case "impact": {
            ...
            if (!resolved)
                return { out: notFound(primary, target), code: 1 };
        ...
        case "layer": {
            ...
            return emit({ ...layer, files }, `${layer.name} — ${files.length} files\n\n${layer.description}\n\n${files.join("\n")}`);
        }
        case "stale": {
            const { head, built, changedFiles } = staleness(primary);
            if (!built)
                return { out: "The map records no commit, so freshness cannot be checked.", code: 0 };
            if (head === built)
                return { out: `Map is current (${built.slice(0, 7)}).`, code: 0 };
            if (changedFiles === null) {
                return { out: `Map built at ${built.slice(0, 7)}; git cannot compare it to HEAD.`, code: 0 };
            }
            return emit({ built, head, changedFiles }, changedFiles.length === 0
        ...
        case "tour": {
            const tour = (primary.graph.tour ?? []);
            if (tour.length === 0)
                return { out: "This map has no tour.", code: 0 };
            return emit(tour, tour.map((s) => `${s.order}. ${s.title}\n${s.description}`).join("\n\n"));
        }
        case "doctor": {
