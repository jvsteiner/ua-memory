 ...
 */
export const NO_DEPENDENTS_CAVEAT = "No static dependents found. This is not proof the file is unused. The map " +
 ...
 */
export function impact(map, path, depth = 1) {
    const start = resolveFile(map, path);
    if (!start)
        return [];
    const seenFiles = new Set([start]);
    const hits = [];
    let frontier = new Set((map.byPath.get(start) ?? []).map((n) => n.id));
    for (let level = 1; level <= depth; level++) {
        const next = new Set();
        for (const edge of map.graph.edges) {
            if (!frontier.has(edge.target))
                continue;
            if (!DEPENDENCY_EDGES.has(edge.type))
                continue;
            const source = map.byId.get(edge.source);
            if (!source || seenFiles.has(source.filePath))
                continue;
            ...
            for (const node of map.byPath.get(source.filePath) ?? [])
                next.add(node.id);
        }
        if (next.size === 0)
            break;
 ...
 */
export function where(map, text, limit = 20) {
...
export function describeFile(map, path) {
    ...
    for (const edge of map.graph.edges) {
        ...
        if (ownIds.has(edge.source)) {
            ...
            if (target && target.filePath !== resolved)
                dependsOn.add(target.filePath);
        }
        if (ownIds.has(edge.target)) {
            const source = map.byId.get(edge.source);
            if (source && source.filePath !== resolved)
                dependedOnBy.add(source.filePath);
        }
    }
    return {
        path: resolved,
...
export function findLayer(map, name) {
    ...
    return (map.graph.layers.find((l) => l.name.toLowerCase() === needle) ??
...
export function filesInLayer(map, layer) {
